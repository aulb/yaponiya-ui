// TODO: Cleanup constants.js, utils.js
import { palette } from '../helpers/palette';

export const NUMBER_OF_KANJI = 2136;

// https://github.com/jamesknelson/node-joyo-kanji/blob/master/index.js
export const ORDER = {
  /* Categorical order bys */
  GRADE: 'grade',
  JLPT: 'jlpt',
  STROKE_COUNT: 'stroke_count', // Legacy non camel case
  /* Sequential order bys */
...
export const determineFontColor = (hexColor) => {
  const color = hexToRgb(hexColor);

  const lightness = 255 - ((0.299 * color.r) + (0.587 * color.g) + (0.114 * color.b));
  if (lightness < 127.5) {
    return '#000';
  }
  return '#fff';
...
/* Hardcoded solution for coloring */
...
export const getOrderType = (order) => {
  switch (order) {
    ...
    case ORDER.JLPT:
      return ORDER_TYPE.CATEGORICAL;
    case ORDER.HEISIG:
    case ORDER.NEWS:
    case ORDER.TWEET:
    default:
      return ORDER_TYPE.SEQUENTIAL;
  }
};

export const getUniqueOrderTypeAmount = (order) => {
  switch (order) {
    case ORDER.GRADE:
      return 8;
    case ORDER.STROKE_COUNT:
    ...
    case ORDER.TWEET:
    default:
      return NUMBER_OF_KANJI;
  }
};

export const getPalette = (order) => {
  const currentOrderType = getOrderType(order);
  const uniqueAmount = getUniqueOrderTypeAmount(order);
  if (currentOrderType === ORDER_TYPE.CATEGORICAL) {
    return palette('tol-rainbow', uniqueAmount);
  }
  return palette('tol-sq', uniqueAmount);
};

export const isAmountValid = (order, amount) => {
  if (amount === Number.MAX_SAFE_INTEGER || amount === Number.MIN_SAFE_INTEGER) return false;
  const acceptableRange = getUniqueOrderTypeAmount(order);
  return amount <= acceptableRange && amount >= 0;
};
/* End hardcoded solution for coloring */
...
export const FLASH_COLOR = '#FFFF8D';
export const UNDEFINED_COLOR = '#263238';
